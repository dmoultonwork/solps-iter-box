function grid = add_mag_field(grid,input)
    ...
    grid.fratio = grid.Bp(ix,iy)/sqrt(grid.Bp(ix,iy)^2+grid.Bt(ix,iy)^2); % B_poloidal/B_total
    % y-y_sep at divertor entrance:
    ymysep_entrance = [0,cumsum(sqrt(diff(grid.rc(1,:)).^2+diff(grid.zc(1,:)).^2))]-sum(grid.dy(1,1:iysep-1));
    % y-y_sep at midplane:
